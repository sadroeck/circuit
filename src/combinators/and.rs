...
impl<L, R> Proc for AndThenProc<L, R>
where
    L: Proc + Send,
    R: Proc + Send,
{
...
impl<L, R> Drop for AndThenProc<L, R>
where
    L: Proc + Send,
    R: Proc + Send,
{
...
mod test {
    ...
    fn join_on_drop() {
        ...
        let joined = left.and_then(right);
        drop(joined);
    ...
    fn nested() {
        ...
        let joined = left.and_then(middle).and_then(right);
        drop(joined);
